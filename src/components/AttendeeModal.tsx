...
export const AttendeeModal: React.FC<AttendeeModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ form: err.message || 'Failed to save attendee' });
    } finally {
  ...
  return (
    <div id="attendee-modal-backdrop" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
      <div id="attendee-modal-content" className="bg-white border border-slate-200 rounded-xl max-w-md w-full shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {errors.form && (
            <div id="attendee-form-error-banner" className="p-3 rounded-md bg-rose-50 border border-rose-200 text-rose-700 text-sm flex items-center gap-2">
          ...
          {/* Full Name */}
          <div>
            <label htmlFor="attendee-name-input" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">
          ...
          {/* Email */}
          <div>
            <label htmlFor="attendee-email-input" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">
            ...
            <input
              ...
              className={`w-full px-3.5 py-2 bg-white border rounded-md text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-1 shadow-2xs ${
                errors.email ? 'border-rose-400 focus:ring-rose-500' : 'border-slate-200 focus:ring-indigo-500 focus:border-indigo-500'
              }`}
            />
          ...
          {/* Phone */}
          <div>
            <label htmlFor="attendee-phone-input" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">
          ...
          {/* Organization */}
          <div>
            <label htmlFor="attendee-org-input" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">
